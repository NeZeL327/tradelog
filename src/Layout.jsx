  ...
  return (
    <SidebarProvider defaultOpen={!isMobile}>
      <div className="min-h-screen flex w-full bg-background">
        ...
        >
          {/* Logo */}
          <SidebarHeader className="border-b border-sidebar-border px-5 py-5 group-data-[collapsible=icon]:px-3 group-data-[collapsible=icon]:py-4">
            <div className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
              <div className="logo-arrow logo-arrow-square w-[40px] h-[40px] flex-shrink-0 rounded-xl">
              ...
              <div className="group-data-[collapsible=icon]:hidden min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Trading Journal</p>
              </div>
            </div>
          </SidebarHeader>

          {/* Navigation Groups */}
          <SidebarContent className="flex-1 overflow-y-auto px-3 py-3">
            {navGroups.map((group) => (
              <SidebarGroup key={group.label} className="mb-1">
                <SidebarGroupLabel className="text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-widest px-3 py-2 group-data-[collapsible=icon]:hidden">
                  {group.label}
                </SidebarGroupLabel>
                <SidebarGroupContent>
                  <SidebarMenu>
                      ...
                      return (
                        <SidebarMenuItem key={item.title}>
                          <SidebarMenuButton
                            ...
                            tooltip={item.title}
                            className={`
                              relative rounded-lg mb-0.5 transition-all duration-150
                              ${isActive
                          ...
                          >
                            <Link
                              to={item.url}
                              className="flex items-center gap-3 px-3 py-2.5 group-data-[collapsible=icon]:px-2 group-data-[collapsible=icon]:justify-center"
                            >
          ...
          <SidebarFooter className="border-t border-sidebar-border px-3 py-3">
            {user && (
              <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-sidebar-accent transition-colors cursor-default group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0">
                ...
                <button
                  onClick={() => logout()}
                  className="group-data-[collapsible=icon]:hidden p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-background/50 transition-colors flex-shrink-0"
                  title={t('logout')}
                >
