...
export default function TradeFormNew({ trade = null, onSuccess, onClose, defaultStatus = "Open", embedded = false }) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      psychology_tags: Array.isArray(trade.psychology_tags) ? trade.psychology_tags : [],
  ...
  const psychologyOptions = useMemo(() => {
    const base = tagLists.psychology || [];
    const extras = (formData.psychology_tags || []).filter(
      (t) => !base.some((b) => String(b).toLowerCase() === String(t).toLowerCase())
    );
    return extras.length ? [...base, ...extras] : base;
  }, [tagLists.psychology, formData.psychology_tags]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = stripUndefined({
        ...
        psychology_tags: Array.isArray(formData.psychology_tags) ? formData.psychology_tags : [],
          ...
          <CardContent className={embedded ? "p-0" : "p-3 sm:p-4"}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <EditableTagChips
                kind="psychology"
                accent="violet"
                label="Psychologia / mindset"
                icon={Brain}
                options={psychologyOptions}
                selected={formData.psychology_tags}
                onToggle={(tag) => toggleChip("psychology_tags", tag)}
                onOptionsChange={(psychology) => persistTagLists({ psychology })}
                onSelectedChange={(psychology_tags) =>
                  setFormData((prev) => ({ ...prev, psychology_tags }))
                }
              />
