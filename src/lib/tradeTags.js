...
/** Mindset / psychology tags (editable; separate from emotion stages). */
export const DEFAULT_PSYCHOLOGY = Object.freeze([
  "Spokój / dyscyplina",
  "FOMO mental",
  "Revenge mindset",
  "Overconfidence",
...
export function resolvePsychology(settings) {
  return normalizeTagList(settings?.trade_psychology, DEFAULT_PSYCHOLOGY);
}
...
export function loadTradeTagLists({ cloudSettings } = {}) {
  ...
  return {
    ...
    psychology: resolvePsychology(effective),
  };
}

/** Persist vocabulary locally (+ Firestore when logged in). */
export async function saveTradeTagLists({ confluences, mistakes, psychology, userId } = {}) {
  const local = loadLocalUserSettings();
  const next = {
    trade_confluences:
      confluences !== undefined
        ? normalizeTagList(confluences, DEFAULT_CONFLUENCES)
        : resolveConfluences(local),
    trade_mistakes:
      mistakes !== undefined
        ? normalizeTagList(mistakes, DEFAULT_MISTAKES)
        : resolveMistakes(local),
    trade_psychology:
      psychology !== undefined
        ? normalizeTagList(psychology, DEFAULT_PSYCHOLOGY)
        : resolvePsychology(local),
  };
  ...
  return {
    ...
    psychology: next.trade_psychology,
  };
}
